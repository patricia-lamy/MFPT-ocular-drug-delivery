...
problem = {
    ...
    'names': ['dummy', 'a', 'b', 'lens_diam', 'lens_thick', 'h_va',
              'D', 'k_va', 'k_vr'],
    'bounds': [[0, 1],
               [0.9*0.895, 1.1*0.895],        # cm
               [0.9*0.678, 1.1*0.678],          # cm
               [0.9*0.75, 1.1*0.75],          # cm
               [0.9*0.351, 1.1*0.351],        # cm
               [0.9*0.163, 1.1*0.163],          # cm (pm 10% on hva=0.251 cm)
...
for j in range(0, nb_chuncks):
    ...
    df.to_csv("data/2024-06-20_cyno_Ns337_10percent.csv", index=False,
              header=(j == 0), mode='a')
