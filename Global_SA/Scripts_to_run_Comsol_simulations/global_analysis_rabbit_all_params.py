...
problem = {
    'num_vars': num_vars,
    'names': ['dummy', 'b', 'h_va', 'D', 'k_va', 'k_vr'],
    'bounds': [[0, 1],
               [0.566, 0.611],          # cm
               [0.2142, 0.2618],        # cm
...
for j in range(0, nb_chuncks):
    ...
    for i in range(0, chunk_size):
        ...
        # Assigning variables
        a = 0.9
        b = param_values[position_param, 1]
        lens_diam = 0.995
        lens_thick = 0.66
        h_va = param_values[position_param, 2]
        ...
        model.parameter('D', str(param_values[position_param, 3]))
        model.parameter('hya_perm', str(param_values[position_param, 4]))
        model.parameter('ilm_perm', str(param_values[position_param, 5]))
        # model.build()
        # print(model.parameters())
        model.solve()
    ...
    df = pd.DataFrame({'#b': param_values[l_bd:h_bd, 1],
                       '#Height va int': param_values[l_bd:h_bd, 2],
                       '#Diffusion coef': param_values[l_bd:h_bd, 3],
                       '#Perm va': param_values[l_bd:h_bd, 4],
                       '#Perm vr': param_values[l_bd:h_bd, 5],
    ...
    df.to_csv("data/2024-06-04_rabbit_Ns257_reduced_params_D_perms.csv",
              index=False,
              header=(j == 0), mode='a')
